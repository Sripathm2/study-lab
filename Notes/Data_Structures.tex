...
\hypertarget{job_sequencing_deadlines}{%
\subsection{Job\_sequencing\_deadlines}\label{job_sequencing_deadlines}}

\emph{(Skeleton stage --- written for the intended algorithm; reconcile
against the code once implemented.)}

\textbf{Problem.} n jobs, each taking exactly 1 time unit, each with a
deadline \texttt{d{[}i{]}\ \textgreater{}=\ 1} and profit
\texttt{p{[}i{]}}. One machine, one job per slot \texttt{1,\ 2,\ 3,\ …};
a job pays only if scheduled in a slot ≤ its deadline. Maximize total
profit (the Abdul Bari greedy classic).

\hypertarget{idea-16}{%
\subsubsection{Idea}\label{idea-16}}

Two greedy decisions, both provable by exchange arguments: -
\textbf{Order: profit descending.} If an optimal schedule skips a
higher-profit job in favor of a lower-profit one competing for the same
slots, swapping them can't decrease profit. - \textbf{Placement: the
LATEST free slot ≤ the deadline.} Placing a job as late as legally
possible keeps early slots open for jobs with tighter deadlines. The
\texttt{d\{4,1,1,1\}\ p\{50,10,40,30\}} case is the demonstration: put
50 in slot 4 (not slot 1) and slot 1 stays free for 40 → 90; greedily
grabbing slot 1 for the 50 strands every deadline-1 job → 60.

A job finding no free slot ≤ its deadline is simply dropped (it can
never pay).

Feasibility fact worth knowing (it powers the brute-force verifier): a
\emph{set} of unit jobs is schedulable iff, after sorting its deadlines
ascending, the i-th deadline (1-based) is ≥ i --- Hall's condition
specialized to intervals \texttt{{[}1,\ d{]}}.

\hypertarget{complexity-16}{%
\subsubsection{Complexity}\label{complexity-16}}

O(n log n) sort + O(n · maxD) slot probing with a plain free-slot array
(each job scans backward from its deadline). The classic upgrade: a
\textbf{union-find over slots} where \texttt{find(d)} returns the latest
free slot ≤ d and placing unions it with slot−1 --- O(n log n + n·α)
total, and a nice reuse of the repo's \texttt{Union\_find\_compressed}.
Cap slot bookkeeping at \texttt{min(maxDeadline,\ n)} --- no schedule
ever uses more than n slots (deadlines can be huge; the runner's
million-deadline case guards this).
...
\tightlist
\item
  \textbf{Contrast with 0/1 knapsack:} near-identical flavor (``pick
  items for profit under a constraint'') but here greedy is
  \emph{optimal} while knapsack's greedy fails and needs DP. The
  difference is the matroid structure of the schedulable sets --- the
  interview-grade observation.
\item
  The maximum \textbf{profit} is unique; the chosen job \emph{set} is
  not (equal profits swap freely) --- assert profit, and assert counts
  only where they're forced (all-deadline-1 → exactly one job runs).
\item
  Weighted variants with unequal job lengths lose the matroid structure
  and go NP-hard --- this problem lives right on the tractability
  boundary.
\end{itemize}
...
\hypertarget{n_queens}{%
\subsection{N\_queens}\label{n_queens}}

\emph{(Skeleton stage --- written for the intended algorithm; reconcile
against the code once implemented.)}

\begin{equation*}
\text{two queens }(r_1,c_1),(r_2,c_2)\text{ conflict}\iff
c_1=c_2 \ \lor\ r_1-c_1=r_2-c_2 \ \lor\ r_1+c_1=r_2+c_2
\end{equation*}

\textbf{Problem.} Place n queens on an n×n board with no two sharing a
row, column, or diagonal. Count all distinct placements, and produce
one.

\hypertarget{idea-19}{%
\subsubsection{Idea}\label{idea-19}}

\textbf{Backtracking} --- the exhaustive search that prunes instead of
enumerating. One queen per row is forced, so search row by row: at row
r, try each column c that is unattacked, place, recurse to r+1,
\textbf{undo}, try the next c.~Reaching row n is one complete solution.
The undo is the essence: state must be bit-identical before and after
each attempt, or ``solutions'' leak between branches.

O(1) attack tests via three occupancy sets --- the observation that
makes the search fast: - \texttt{cols{[}c{]}} --- columns are just c; -
\texttt{diag1{[}r\ +\ c{]}} --- every ``/'' anti-diagonal has constant
r+c (2n−1 of them); - \texttt{diag2{[}r\ −\ c\ +\ n\ −\ 1{]}} --- every
``" diagonal has constant r−c, shifted non-negative.

Place = set three booleans; undo = clear them. Without these, each
attack check is an O(n) rescan of placed queens.

For \texttt{oneSolution}, the same recursion short-circuits on the first
success (return/flag up the stack); n = 2 and n = 3 are the only
unsolvable sizes ≥ 1 --- return empty, and keep
\texttt{countSolutions\ ==\ 0\ ⇔\ oneSolution\ empty} as a consistency
invariant (the runner checks it).

\hypertarget{complexity-19}{%
\subsubsection{Complexity}\label{complexity-19}}

Worst-case O(n!) branching (row r has ≤ n−r viable columns), heavily
pruned in practice; counting n = 9 (352) is instant, n ≈ 15 is where it
starts to breathe hard. Space O(n) for the sets + recursion.
...
\tightlist
\item
  \textbf{Counting convention:} rotations/reflections count separately
  (n = 8 → 92 total, vs 12 ``fundamental'' solutions). The count
  sequence 1, 0, 0, 2, 10, 4, 40, 92, 352 is OEIS A000170 --- the
  runner's independently established constants.
\item
  The classic halving optimization: first-row symmetry (mirror pairs)
  lets you search half the columns and double --- worth doing only after
  the plain version is trusted.
\item
  Backtracking vs DP: subproblems here (partial boards) don't repeat, so
  there's nothing to memoize --- this is exhaustive search with pruning,
  the third technique next to D\&C and DP, and the template for Sudoku,
  subset-sum search, and graph coloring. Bitmask sets (\texttt{int}
  instead of \texttt{boolean{[}{]}}) are the constant-factor upgrade.
\end{itemize}
